...
interface SearchBarProps {
  onAddBookmark: (item: SearchResult) => void;
  isVisible: boolean;
  bookmarks: Array<{id: number; type: "movie" | "tv"}>;
}
...
const SearchBar = memo(function SearchBar({
  ...
  const searchMoviesAndSeries = async (query: string) => {
    ...
    try {
      ...
      const moviesWithPosters = (moviesData || [])
        .filter((movie: TMDbMovie) => movie.poster_path && movie.release_date)
        .slice(0, 4);

      const seriesWithPosters = (seriesData || [])
        .filter(
          (series: TMDbSeries) => series.poster_path && series.first_air_date,
        )
        .slice(0, 3);

      // Process results in parallel with better error handling
      const allResults = await Promise.allSettled([
        ...moviesWithPosters.map(async (movie: TMDbMovie) => {
          try {
        ...
        ...seriesWithPosters.map(async (series: TMDbSeries) => {
          try {
  ...
  const handleAddBookmark = (item: SearchResult) => {
    onAddBookmark(item);
    setSearchTerm("");
    setResults([]);
    setShowResults(false);
