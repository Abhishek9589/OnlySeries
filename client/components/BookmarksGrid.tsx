...
import { X, Clock, Star, Film, Tv, Calendar, Play } from "lucide-react";
...
interface BookmarksGridProps {
  ...
  onRemoveBookmark: (id: number, type: "movie" | "tv") => void;
...
export default function BookmarksGrid({
  ...
  const formatWatchTime = (item: BookmarkItem) => {
    if (item.type === "movie") {
  ...
  const groupedBookmarks = bookmarks.reduce(
    ...
    {
      franchises: {} as Record<string, BookmarkItem[]>,
      individual: [] as BookmarkItem[],
    },
  );

  // Calculate average rating for franchises
  const getAverageRating = (movies: BookmarkItem[]): string => {
